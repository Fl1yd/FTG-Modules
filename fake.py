...
class FakeActionsMod(loader.Module):
    ...
    async def fakecmd(self, event):
        ...
        actions = ['typing', 'contact', 'game', 'location', 'record-audio', 'record-round',
                   'record-video', 'voice', 'round', 'video', 'photo', 'document', 'cancel']
        args = utils.get_args_raw(event).split()
        if len(args) == 0:
            fake_action = choice(actions)
            ...
            await event.edit('Неправильный ввод.')
            return
        try:
            await event.delete()
            async with event.client.args(event.chat_id, fake_action):
